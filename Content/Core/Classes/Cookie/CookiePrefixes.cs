...
namespace TLR.Content.Core.Classes.Cookie
{
	...
	public class Warm : ModPrefix
	{
        ...
        public override void ApplyAccessoryEffects(Player player) { player.GetModPlayer<TLRPlayer>().cookieBaking += 1; }
        public override IEnumerable<TooltipLine> GetTooltipLines(Item item) {
			yield return new TooltipLine(Mod, "PrefixAccHealth", "+1 cookies / second") {
				IsModifier = true, // Sets the color to the positive modifier color.
			};
		}
    }
	public class Heated : ModPrefix
	{
		public override PrefixCategory Category => PrefixCategory.Accessory;
		public override float RollChance(Item item) => 0.50f;
		public override bool CanRoll(Item item) => true;
		public override void ModifyValue(ref float valueMult) { valueMult *= 1.5f; }
        public override void ApplyAccessoryEffects(Player player) { player.GetModPlayer<TLRPlayer>().cookieBaking += 2; }
		public override IEnumerable<TooltipLine> GetTooltipLines(Item item) {
			yield return new TooltipLine(Mod, "PrefixAccHealth", "+2 cookies / second") {
				IsModifier = true, // Sets the color to the positive modifier color.
			};
		}
    }
